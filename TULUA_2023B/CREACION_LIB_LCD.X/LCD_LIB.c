...
void lcd_init(void){
    ...
    _LCD_DDR &=   ~( (1<<_LCD_RS) |
                     ...
                     (1<<_LCD_D6) |
                     (1<<_LCD_D7) );
    //iniciar todos los pines en cero
    _LCD_PORT &=   ~( (1<<_LCD_RS)|
                     (1<<_LCD_EN) |
                     (1<<_LCD_D4) |
                     (1<<_LCD_D5) |
    ...
    lcd_command(_LCD_FUNTIONSET | _LCD_4BITMODE | _LCD_2LINE);
    _delay_ms(2);
...
void lcd_scroll_left(void){
    lcd_command(_LCD_CURSORDISPLAYSHIFT | _LCD_MOVELEFT | _LCD_DISPLAY_SHIFT);
    _delay_us(37);
}
void lcd_scroll_right(void){
    lcd_command(_LCD_CURSORDISPLAYSHIFT | _LCD_MOVERIGHT | _LCD_DISPLAY_SHIFT );
    _delay_us(37);
}
